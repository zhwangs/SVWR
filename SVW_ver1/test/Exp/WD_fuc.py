    ...
    try:
        if np.isnan(WingerD_out_test):
            ...
            if m==n:
                ...
                WingerD_out=torch.tensor(1.0,requires_grad=True)
            else:
            
                WingerD_out=torch.tensor(0.0,requires_grad=True)
...
def pi_mn(nu,m,n):
    ...
    pi_val_test=pi_val.detach().numpy()
    try:
        if np.isnan(pi_val_test):
            pi_val=torch.tensor(0.0,requires_grad=True)
...
def tau_mn(nu,m,n):
   ...
   # print(x.shape)
    ...
    SP_test=SP.detach().numpy()
    try:
        if np.isnan(SP_test):
            SP=torch.tensor(0.0,requires_grad=True)
...
def Emap_K(Geo_mesh,geo_mag,Geo_mesh_polar,K_mesh,K_mag,n_max=6,polar_psi=0,polar_chi=0,E0=1):
    count=0
    Error_x_array=[]
    Error_y_array=[]
    Error_z_array=[]

    for k_num in range(0,K_mesh.shape[1]):
        Current_k=K_mesh[:,k_num]*K_mag
        K_size=np.sqrt(np.sum(Current_k**2))
        c=3e8
        Omega = c*K_size
        
        K_polar=plane_wave_dir(polar_psi,polar_chi,E0,Current_k)
        
        # plane_wave and plane wave rec 

        # plane-wave 
        x=Geo_mesh[0]
        y=Geo_mesh[1]
        z=Geo_mesh[2]
        RB=plane_wave_grid(x,y,z,K_polar,Current_k,omega=Omega,t0=0)
        ...
        # plane wave rec 
        radius=Geo_mesh_polar[0,:,:]
        theta=Geo_mesh_polar[1,:,:]
        phi=Geo_mesh_polar[2,:,:]
        ...
        E_rec=rec_plane_wave(radius_tensor,theta_tensor,phi_tensor,Current_k,K_polar,n_max)
        ...
        Error_y=np.sum((E_y_r_rec.real-E_y_r.real)**2/np.abs(E_y_r_rec.real+E_y_r.real))/E_y_r_rec.reshape(-1).shape[0]
        Error_z=np.sum((E_z_r_rec.real-E_z_r.real)**2/np.abs(E_z_r_rec.real+E_z_r.real))/E_z_r_rec.reshape(-1).shape[0]
        ...
        Error_z_array.append(Error_z)

    Array_size=int(np.sqrt(len(Error_x_array)))
    Error_x_array=np.array(Error_x_array).reshape(Array_size,Array_size)
    Error_y_array=np.array(Error_y_array).reshape(Array_size,Array_size)
    Error_z_array=np.array(Error_z_array).reshape(Array_size,Array_size)

    return Error_x_array,Error_y_array,Error_z_array
...
def plane_wave_dir(polar_psi,polar_chi,E0,k,direction=False):
    ...
    if len(check_ort)>1:
        ...
        if np.abs(e_a).max()>100:
            check_arg=np.abs(e_a).argmax()
