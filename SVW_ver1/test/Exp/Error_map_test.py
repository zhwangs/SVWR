...
def error_map(n_max, polar_psi, polar_chi,E0,K, K_mag_range,theta, phi, radius,radius_mag_range):
    ...
    for k_mag_index in range(0,len(K_mag_range)):
        ...
        for r_mag_index in range(0,len(radius_mag_range)):
            ...
            Error_x=np.sum((E_x_r.real-E_rec_x.real)**2/np.abs(E_x_r.real+E_rec_x.real))/E_rec_x.reshape(-1).shape[0]
            Error_y=np.sum((E_x_r.real-E_rec_x.real)**2/np.abs(E_y_r.real+E_rec_y.real))/E_rec_y.reshape(-1).shape[0]
            Error_z=np.sum((E_x_r.real-E_rec_x.real)**2/np.abs(E_z_r.real+E_rec_z.real))/E_rec_z.reshape(-1).shape[0]
            print(Error_x)
...
K=kk*np.array([1,0,-1])
n_max=5
K_mag_range=np.linspace(0.01,1,20)#np.array([0.01,0.1,1,10])
radius_mag_range=np.linspace(10,0.1,20)#np.array([1000,100,10,1])

polar_psi=np.pi/4#np.pi/2#np.pi/3
polar_chi=0
